...
一个用 Flutter 写的课程表应用，主要面向大学课表这种按周、按节次排课的场景。现在重点支持 Android，其他平台目录是 Flutter 项目默认生成的，还没有专门适配。
...
- 新增课表口令导入导出，可在一台设备生成口令、另一台设备快速恢复。
- 导入支持同名课表覆盖或另存为新课表。
- 设置页版本号改为从系统读取真实构建版本，不再硬编码。
- 应用版本号更新为 1.1.2。

### v1.1.1

- 适配安卓平板（16:9 / 16:10 / 4:3）横竖屏布局。
- 平板横屏下添加课程页切换为双栏布局。
- 课程表在宽屏下自动限制列宽并居中，避免内容过度拉伸。
